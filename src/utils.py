...
def plot_preprocess_step(data_split, which_ttv, process_function):
    ...
    ax[0].imshow(image)
    ax[1].imshow(process_function(image))
    
    plt.tight_layout()
    plt.show()

def plot_augmentation(data_split, which_ttv, transform, process_function=None):
    '''
    Plot a random image from the data_split and apply 'transform' to four instances of the image, plotting the resulting augmentations
    Arguments:
        data_split (dictionary) - dictionary containing keys 'train', 'val', and 'test' with values being the DataFrame of the split
        which_ttv (string) - 'train', 'val' or 'test'
        transform (callable) - an albumentations transform
        process_function (optional, callable) - the preprocessing function to be applied on an image
    '''
    if which_ttv not in ['train', 'val', 'test']:
        print(f'{which_ttv} is not a valid argument')
        return
    ...
    fig.suptitle(f'Image: {which_ttv}_{random_image_idx}.jpg \nClass: {FER_CLASS_MAP[labels[random_image_idx]]}', y=0.9)
    
    # Plot original image
    original_image = images[random_image_idx]
    if process_function:
        original_image = process_function(original_image)
